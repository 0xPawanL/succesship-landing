...
export default function ProductsSection() {
  ...
  return (
    <section id="products" className="py-16 sm:py-20 bg-white dark:bg-gray-900" ref={sectionRef}>
      <div className="container mx-auto px-3 sm:px-4">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6">Our Products</h2>
        ...
        <div className="max-w-6xl mx-auto">
          <Tabs defaultValue={products[0].name.toLowerCase()} className="w-full">
            {/* Unified responsive tabs that wrap */}
            <TabsList className="flex flex-wrap justify-center items-center gap-5 bg-orange-50 dark:bg-gray-800 p-2 rounded-lg mb-6 sm:mb-8 w-full">
              {products.map((product, index) => (
                <TabsTrigger
                  key={index}
                  value={product.name.toLowerCase()}
                  className="flex-shrink-0 px-3 py-2 text-xs sm:text-sm md:text-base whitespace-nowrap data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:text-orange-600 dark:data-[state=active]:text-orange-400 data-[state=active]:shadow-sm rounded-md transition-all duration-200"
            ...
            {products.map((product, index) => (
              ...
              >
                <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
                  ...
                  <div className="lg:col-span-2">
                    <div className="mb-6">
                      <div className="flex items-center gap-3 sm:gap-4 mb-4">
                        <div
                          className={`w-10 h-10 sm:w-12 sm:h-12 bg-${product.color}-50 dark:bg-${product.color}-900/30 rounded-lg flex items-center justify-center text-${product.color}-600 dark:text-${product.color}-400`}
                        >
                          {product.icon}
                        </div>
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">{product.title}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2 mb-4">
                        {product.comingSoon ? (
                          <Badge className="bg-yellow-100 dark:bg-yellow-900 text-yellow-600 dark:text-yellow-400 border-yellow-200 dark:border-yellow-700 text-xs">
                            <Clock className="w-3 h-3 mr-1" />
                            Coming Soon
                          </Badge>
                        ) : (
                          <Badge className="bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400 border-green-200 dark:border-green-700 text-xs">
                            Available
                          </Badge>
                        )}
                      </div>
                      ...
                      {/* Video Preview */}
                      <div
                        className="bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-xl p-4 sm:p-6 mb-6 cursor-pointer hover:shadow-lg transition-all duration-300 group"
                        onClick={() => openVideoModal(product.name)}
                      >
                        <div className="flex items-center justify-center">
                          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white dark:bg-gray-600 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                            <PlayCircle className="w-6 h-6 sm:w-8 sm:h-8 text-orange-500 dark:text-orange-400" />
                          </div>
                        </div>
                        <div className="text-center mt-3 sm:mt-4">
                          ...
                          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">3 min overview</p>
                        </div>
                      </div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {product.features.map((feature, idx) => (
                            <div key={idx} className="flex items-start">
                              <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-orange-500 dark:text-orange-400 mr-3 mt-0.5 flex-shrink-0" />
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                        <Button
                          className="bg-orange-500 hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-700 text-white w-full sm:w-auto"
                          onClick={() =>
                        ...
                        <Button
                          variant="outline"
                          className="border-orange-300 dark:border-orange-600 text-orange-600 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-950 w-full sm:w-auto"
                          onClick={() =>
                        ...
                        <Button
                          variant="outline"
                          className="border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 w-full sm:w-auto"
                          onClick={() =>
                  ...
                  <div className="bg-orange-50 dark:bg-gray-800 rounded-xl p-4 sm:p-6">
                    <h4 className="font-semibold text-gray-900 dark:text-white mb-4 text-base sm:text-lg">
                      Business Impact
                    </h4>
                    <div className="space-y-3 sm:space-y-4 mb-6">
                      {product.benefits.map((benefit, idx) => (
                        <div
                          key={idx}
                          className="bg-white dark:bg-gray-700 p-3 sm:p-4 rounded-lg shadow-sm flex items-center"
                        >
                          <div className="p-2 bg-orange-100 dark:bg-orange-900 rounded-full text-orange-600 dark:text-orange-400 mr-3 flex-shrink-0">
                    ...
                    <div className="p-3 sm:p-4 border border-orange-200 dark:border-orange-700 rounded-lg bg-orange-50 dark:bg-orange-900/20 mb-4">
                    ...
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-bold text-orange-600 dark:text-orange-400 mb-1">ROI</div>
